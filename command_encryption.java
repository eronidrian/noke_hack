...
class CommandEncryptor {
    private static String offlineUnlockCmd = "";
    private static String offlineKey = "";
    transient static String session = "";
    public static String offlineUnlock() {
        if (offlineUnlockCmd.length() == 40 && offlineKey.length() == 32) {
            byte unlockCommand[] = hexToBytes(offlineUnlockCmd);
            ...
            byte preSessionKey[] = hexToBytes(offlineKey);
            byte sessionBytes[] = hexToBytes(session);

            for (int x = 0; x < preSessionKey.length; x++) {
                int total = toUnsigned(preSessionKey[x]) + toUnsigned(sessionBytes[x]);
                System.out.println(toUnsigned(preSessionKey[x]));
                System.out.println(toUnsigned(sessionBytes[x]));
            ...
            System.out.println(bytesToHex(cmddata));
            System.out.println(bytesToHex(preSessionKey));
            System.arraycopy(encryptPacket(preSessionKey, cmddata), 0, commandpacket, 4, 16);

     return bytesToHex(commandpacket);   
    ...
    private static byte[] encryptPacket(byte combinedkey[], byte data[]) {
        byte buffer[] = new byte[16];
        byte tmpkey[] = new byte[16];
        System.arraycopy(combinedkey, 0, tmpkey, 0, 16);

        aes_enc_dec(data, tmpkey, (byte) 1);
...
//            if ((value >> 7) > 0)
    ...
    public static void main(String[] args) {
        System.out.println(offlineUnlock());
    }
}
